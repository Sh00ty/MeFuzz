...
use libafl::{bolts::{
    ...
    TracingStage,
}, state::{HasCorpus, HasMetadata, StdState}, Error, inputs};
...
pub fn main() {
    ...
    let input_file:String = res.get_one::<String>("input")
        .unwrap()
        .to_string()
        .parse()
...
fn collect_coverage(
    ...
    let inp = &inputs::BytesInput::from(input);
    let exit_kind = executor.run_target_once(&inp).unwrap();
    dbg!(exit_kind);
    dbg!(edges_observer.as_mut_slice());
